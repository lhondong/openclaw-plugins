...
export function createSpanOperations(tracer, openSpans, pluginConfig, api) {
  ...
  function cleanupExpiredSpans() {
    ...
    for (const spanId of expiredSpanIds) {
      ...
      if (span) {
        ...
        api.logger?.warn?.(
          `${LOG_PREFIX} Span timeout, cleaning up without export: spanId=${spanId}, trace=${spanContext.traceId}`,
        );
      }
  ...
  function startCleanupInterval() {
    ...
    if (pluginConfig.debug) {
      api.logger?.info?.(
        `${LOG_PREFIX} Started span cleanup interval (check every ${CLEANUP_INTERVAL_MS / 1000}s, timeout after ${SPAN_TIMEOUT_MS / 1000}s)`,
      );
    }
  }
  ...
  function startSpan(spanData) {
    ...
    const normalizedAttributes = Object.fromEntries(
      Object.entries(flattenAttributes(flatAttributes)).map(([key, value]) => [
        key,

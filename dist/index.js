...
import { GEN_AI_INPUT, GEN_AI_OUTPUT, GEN_AI_PROVIDER_NAME, GEN_AI_REQUEST_MODEL, GEN_AI_RESPONSE_MODEL, GEN_AI_SESSION_ID, GEN_AI_SPAN_KIND, GEN_AI_SYSTEM, GEN_AI_USAGE_CACHE_CREATION_INPUT_TOKENS, GEN_AI_USAGE_CACHE_READ_INPUT_TOKENS, GEN_AI_USAGE_INPUT_TOKENS, GEN_AI_USAGE_OUTPUT_TOKENS, GEN_AI_USAGE_TOTAL_TOKENS, GEN_AI_USER_ID, GenAiSpanKind, getGenAiCompletionContent, getGenAiCompletionRole, getGenAiPromptContent, getGenAiPromptRole, } from './utils/genaiConst.js';
import { LANGFUSE_AS_ROOT, LANGFUSE_ENVIRONMENT, LANGFUSE_OBSERVATION_INPUT, LANGFUSE_OBSERVATION_METADATA, LANGFUSE_OBSERVATION_MODEL, LANGFUSE_OBSERVATION_OUTPUT, LANGFUSE_OBSERVATION_STATUS_MESSAGE, LANGFUSE_OBSERVATION_TYPE, LANGFUSE_OBSERVATION_USAGE_DETAILS, LANGFUSE_RELEASE, LANGFUSE_SESSION_ID, LANGFUSE_SESSION_ID_COMPAT, LANGFUSE_TRACE_INPUT, LANGFUSE_TRACE_METADATA, LANGFUSE_TRACE_NAME, LANGFUSE_TRACE_OUTPUT, LANGFUSE_USER_ID, LANGFUSE_USER_ID_COMPAT, LangfuseObservationType, } from './utils/langfuseConst.js';
...
function createTraceAttributes({ name, input, output, userId, sessionId, metadata, environment, }) {
    const attributes = {
...
function createObservationAttributes({ type, input, output, model, metadata, usageDetails, statusMessage, }) {
    const attributes = {
...
function parseCronSessionKey(sessionKey) {
    ...
    const runSessionId = runMarkerIndex >= 0 ? trimmed.slice(runMarkerIndex + runMarker.length).trim() : undefined;
...
function resolveTraceExporterConfig(pluginConfig, logger) {
    if (pluginConfig.langfuseBaseUrl && pluginConfig.langfusePublicKey && pluginConfig.langfuseSecretKey) {
        return {
            ...
            headers: {
                Authorization: buildBasicAuthHeader(pluginConfig.langfusePublicKey, pluginConfig.langfuseSecretKey),
    ...
    if (pluginConfig.endpoint) {
        logger?.warn?.(`${LOG_PREFIX} pluginConfig.endpoint is deprecated. Prefer langfuseBaseUrl/langfusePublicKey/langfuseSecretKey.`);
        return {
...
const langfuseOpenClawPlugin = {
    ...
    register(api) {
        ...
        if (!exporterConfig) {
            api.logger?.warn?.(`${LOG_PREFIX} Missing Langfuse configuration. Set langfuseBaseUrl/langfusePublicKey/langfuseSecretKey.`);
            return;
        }
        ...
        const { startSpan, endSpan, exportSpan, flush, getContextBySession, getContextByConversationId, getContextByRunId, getToolCallInfo, setContextBySession, setContextByConversationId, setContextByRunId, setToolCallInfo, deleteContextBySession, deleteContextByConversationId, deleteContextByRunId, deleteToolCallInfo, createSpanData, startCleanupInterval, stopCleanupInterval, startContextCleanupInterval, stopContextCleanupInterval, createAPMContext, cleanAllContextMaps, } = traceHelpers;
        ...
        function maybeEndRootSpan(traceContext, endTime = Date.now()) {
            ...
            const hasActiveAgent = Object.values(traceContext.allSessionMap || {}).some(sessionState => Boolean(sessionState?.currentAgent));
            if (hasActiveAgent) {
        ...
        function createRootRequestContext({ now, traceName = 'openclaw_request', sessionKey, sessionId, conversationId, channelId, input, eventFrom, metadata = {}, }) {
            ...
            const { traceId, spanId } = startSpan(createSpanData(traceName, 'entry', now, undefined, rootAttributes, 'rootSpan'));
        ...
        function ensureTraceContext({ sessionKey, sessionId, conversationId, channelId, input, eventFrom, traceName, metadata, }) {
            let traceContext = (sessionKey && getContextBySession(sessionKey)) ||
        ...
        function ensureLiveTraceContext({ traceName = 'openclaw_request', sessionKey, sessionId, conversationId, channelId, runId, input, eventFrom, metadata = {}, }) {
            let traceContext = (runId && getContextByRunId(runId)) ||
        ...
        function exportChildSpan({ name, type, startTime = Date.now(), endTime = startTime, parentTraceId, parentSpanId, attributes, }) {
            return exportSpan(createSpanData(name, type, startTime, endTime, attributes, {
        ...
        api.logger?.info?.(`${LOG_PREFIX} Initialized. exporter=${exporterConfig.mode}, url=${exporterConfig.url}`);
        api.on('gateway_start', async (event) => {
            const now = Date.now();
            exportSpan(createSpanData('gateway_start', 'gateway', now, now, {
                ...createTraceAttributes({
                    name: 'gateway_start',
        ...
        api.on('gateway_stop', async (event, runtimeContext) => {
            const now = Date.now();
            exportSpan(createSpanData('gateway_stop', 'gateway', now, now, {
                ...createTraceAttributes({
                    name: 'gateway_stop',
        ...
        api.on('session_start', async (event, runtimeContext) => {
            ...
            const now = Date.now();
            const cronContext = existingContext?.cronContext ??
                resolveCronContext({ sessionKey, trigger: runtimeContext.trigger });
            exportSpan(createSpanData('session_start', 'entry', now, now, {
                ...createObservationAttributes({
                    type: LangfuseObservationType.SPAN,
                    metadata: {
        ...
        api.on('session_end', async (event, runtimeContext) => {
            ...
            const cronContext = traceContext?.cronContext ??
                resolveCronContext({ sessionKey, trigger: runtimeContext.trigger });
            const exported = exportSpan(createSpanData('session_end', 'entry', now, now, {
                ...createObservationAttributes({
                    type: LangfuseObservationType.SPAN,
                    metadata: {
        ...
        api.on('message_sending', async (event, runtimeContext) => {
            ...
            const { spanId } = startSpan(createSpanData('message_send', 'message', now, undefined, {
                ...createObservationAttributes({
                    type: LangfuseObservationType.SPAN,
        ...
        api.on('llm_input', async (event, runtimeContext) => {
            ...
            const { spanId } = startSpan(createSpanData(`llm:${event.provider}/${event.model}`, 'model', now, undefined, {
                ...createObservationAttributes({
        ...
        api.on('before_tool_call', async (event, runtimeContext) => {
            ...
            const cronContext = traceContext.cronContext;
            const sessionState = createSessionState(traceContext, sessionKey);
            const parentSpanId = sessionState.currentLlmInvoke?.spanId ??
            ...
            const { spanId } = startSpan(createSpanData(event.toolName, 'tool', now, undefined, {
                ...createObservationAttributes({
        ...
        api.on('after_tool_call', async (event, runtimeContext) => {
            ...
            if (storedToolCall) {
                ...
                exportChildSpan({
                    ...
                    type: 'tool',
                    parentTraceId: traceContext.traceId,
                    parentSpanId: sessionState.currentAgent?.agentSpanId ??
                        sessionState.totalSessionAncestorSpanId ??
                        traceContext.rootSpanId,
                    attributes,
                });
        ...
        api.on('tool_result_persist', (event, runtimeContext) => {
            ...
            const toolCallId = event.toolCallId ?? runtimeContext.toolCallId;
            const storedToolCall = toolCallId ? getToolCallInfo(toolCallId) : undefined;
            const parentSpanId = storedToolCall?.spanId ??
                sessionState.currentAgent?.agentSpanId ??
                sessionState.totalSessionAncestorSpanId ??
                traceContext.rootSpanId;
            exportChildSpan({
        ...
        api.on('before_model_resolve', async (event, runtimeContext) => {
            ...
            const userId = getRequestUserId(runtimeContext.sessionKey, traceContext.conversationId, runtimeContext.channelId);
            const now = Date.now();
            const { spanId } = startSpan(createSpanData(agentId, 'agent', now, undefined, {
                ...createObservationAttributes({
        ...
        api.on('after_compaction', async (event, runtimeContext) => {
            const traceContext = (runtimeContext.sessionKey && getContextBySession(runtimeContext.sessionKey)) ||
        ...
        api.on('subagent_spawning', async (event, runtimeContext) => {
            ...
            const now = Date.now();
            const { spanId } = startSpan(createSpanData('subagent_spawn', 'subagent', now, undefined, {
                ...createObservationAttributes({
                    type: LangfuseObservationType.AGENT,
                    metadata: {
                        label: event.label,
        ...
        api.on('subagent_spawned', async (event, runtimeContext) => {
            ...
            const lifecycleParentSpanId = createSessionState(traceContext, runtimeContext.requesterSessionKey || childSessionKey).currentAgent
                ?.agentSpanId ?? traceContext.rootSpanId;
            const { spanId } = startSpan(createSpanData('subagent_lifecycle', 'subagent', now, undefined, {
                ...createObservationAttributes({
                    type: LangfuseObservationType.AGENT,
                    metadata: {
                        childSessionKey,
        ...
        if (typeof api.registerHook === 'function') {
            api.registerHook(['command:new', 'command:reset', 'command:stop'], async (payload) => {
                try {
                    const action = payload?.action || 'unknown';
                    const sessionKey = payload?.sessionKey || 'unknown';
                    const traceContext = getContextBySession(sessionKey);
                    const now = Date.now();
                    exportSpan(createSpanData(`openclaw.command.${action}`, 'command', now, now, {
                        ...createObservationAttributes({
                            type: LangfuseObservationType.SPAN,
                            metadata: {
                                action,

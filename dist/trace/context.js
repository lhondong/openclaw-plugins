...
export function createContextManager(pluginConfig, api) {
    ...
    function getOrCreateContextBySession(sessionKey, hookName) {
        return createOrReuseContext(getContextBySession, setContextBySession, sessionKey, hookName, 'sessionId');
    }
    function getOrCreateContextByConversation(conversationId, hookName) {
        return createOrReuseContext(getContextByConversationId, setContextByConversationId, conversationId, hookName, 'conversationId');
    }
    function getOrCreateContextByRunId(runId, hookName) {
        return createOrReuseContext(getContextByRunId, setContextByRunId, runId, hookName, 'runId');
    }
    ...
    function startCleanupInterval() {
        ...
        if (pluginConfig.debug) {
            api.logger?.info?.(`${LOG_PREFIX} Started context cleanup interval (check every ${CLEANUP_INTERVAL_MS / 1000}s, timeout after ${CONTEXT_TIMEOUT_MS / 1000}s)`);
        }
    }
